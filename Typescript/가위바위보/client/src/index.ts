...
const startbtn= document.getElementById('start-button') as HTMLButtonElement; // 시작 버튼
const comEl = document.getElementById('com') as HTMLDivElement; // 가위바위보 출력

let items: CircleData = new CircleData([
    new Item("가위", game),
    new Item("바위", game),
    new Item("보", game)
]);

let currentItem: Item = items.getAll()[0]; // [0]으로 초기화.
let timerId: number;


// body에 button 추가 (렌더링), disabled 값 설정
const item_buttons = document.getElementById('item_buttons') as HTMLDivElement;
items.getAll().forEach(function (item: Item) {
